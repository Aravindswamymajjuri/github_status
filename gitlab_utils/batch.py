...
def process_single_user(client, username, date_range=None):
    """
    ...
    try:
        ...
        # 5. MRs
        user_mrs, mr_stats = merge_requests.get_user_mrs(client, user_id, date_range=date_range)
        result["data"]["mrs"] = user_mrs
        result["data"]["mr_stats"] = mr_stats

        # 6. Issues
        user_issues, issue_stats = issues.get_user_issues(client, user_id, date_range=date_range)
...
def process_batch_users(client, usernames, date_range=None):
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_user = {
            executor.submit(process_single_user, client, u, date_range=date_range): u
