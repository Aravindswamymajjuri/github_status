...
def render_user_profile(client, user_info):  # noqa: C901
    ...
    afternoon_commits = len(
        [c for c in commit_rows if (c.get("slot") or "").lower() == "afternoon"]
    )
    ...
    if not projects:
        ...
        if personal_projects:
            ...
            for p in personal_projects:
                st.markdown(
                    f"- [{p.get('name_with_namespace', p.get('name'))}]({p.get('web_url')})"
                )
