...
def test_suggestions_for_all_projects():
    ...
    suggestion_logic = re.search(
        (
            r"for key, display_name, suggestion_text in suggestion_list:\s*"
            r"if not report\.get\(key, True\):"
        ),
        content,
...
def test_readme_scores_for_all_projects():
    ...
    if not expected_statuses.issubset(actual_statuses):
        print(
            "❌ FAIL: Missing expected readme_status values. "
            f"Expected: {expected_statuses}, Found: {actual_statuses}"
        )
        return False
...
def test_suggestions_called_for_all_projects():
    ...
    if len(suggestion_calls) < 2:  # Should be called in both single project and batch mode
        print(
            "❌ FAIL: Not enough calls to "
            f"get_suggestions_for_missing_items found: {len(suggestion_calls)}"
        )
        return False
